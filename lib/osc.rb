...
def Osc.hadsoftname_description(products_name)
  id=products_name
  id=Misc.txt2Sql(id.to_s) 
  sql="SELECT count( * ) \
      FROM `products_description` \
...
fullhtml= fullhtml + " <tr>\n    <td width=\"115\">Discription</td>\n    <td>"
fullhtml= fullhtml + deschtml
fullhtml= fullhtml + "</td>\n  </tr>\n "
if !(filestablecode=="")
fullhtml= fullhtml + " <tr>\n    <td width=\"115\">Included Files</td>\n    <td>"
fullhtml= fullhtml + filestablecode+ "<blockquote>&nbsp;</blockquote></td>\n  </tr>\n"
...
if self.hasSoftname(products_name).to_s == "0".to_s and self.hasId_products_description(products_id).to_s=="0" and self.hadsoftname_description(products_name).to_s=="0"
self.dbprocess(sql2)
else
...
def Osc.insertDlpathmap(myID)
  ...
  if !(resultarr.nil?)
    puts "no download map inserted to osc"
    return
    else
